#!/bin/bash
# Usage: watch -n5 promoter/status.sh
# Shows promotion status for all PromotionStrategies across sectors
...
for ps in $(kubectl -n "$NAMESPACE" get promotionstrategy -o jsonpath='{.items[*].metadata.name}'); do
  kubectl -n "$NAMESPACE" get promotionstrategy "$ps" -o json | jq -r '
    ...
    "=== " + .metadata.name + " (" + (.spec.activePath // "sector-level") + ") ===",
    (.status.environments[] |
      "  " +
      (.branch | ltrimstr("environment/") | pad(22)) +
